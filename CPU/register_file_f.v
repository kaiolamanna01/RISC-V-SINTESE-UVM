module register_file_f #(
    ...
    always @(posedge CLK or posedge rst) begin
        if (rst) begin
            ...
            // Escreve WD3 no endereÃ§o A3 se a escrita estÃ¡ habilitada
            // Registrador f0 Ã© read-only (sempre zero) - conforme especificaÃ§Ã£o RISC-V
            if (A3 != 0) begin
                registrador[A3] <= WD3;
            end
        end
    ...
    // Leitura assÃ­ncrona
    always @(*) begin
        // Registrador f0 Ã© sempre zero (read-only)
        if (A1 == 0)
            RD1 = 0;
        else
            RD1 = registrador[A1];
