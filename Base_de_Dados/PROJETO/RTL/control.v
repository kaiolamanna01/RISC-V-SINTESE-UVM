module control(
    ...
    // Main Decoder
    // CORREÇÃO: ResultSrc, RegWrite, FPURegWrite precisam ser 'reg'
    ...
    output reg FPURegWrite, // CORREÇÃO: Precisa ser 'reg'
    output FPUResultSrc, 
    output FPUOp,
    ...
    // CORREÇÃO: Declaração dos Wires para os sinais do maindec
    ...
    // Instância do Main Decoder (saídas conectadas aos wires 'md_')
    maindec maindec(
        .op(op),
        .Branch(md_Branch),
    ...
    // Conexões diretas (sinais que 'control' não modifica)
    ...
    // ALU Decoder e Lógica FPU
    always @(*) begin
        // --- Início: Lógica ORIGINAL (para ALUControl e FPUSel) ---
        ALUControl = 3'b000;
        byteEnable = 4'b1111;
        FPUSel = 5'b00000;
        ...
        if (op == 7'h53) begin // Opcode FPU 1010011
            case ({funct7, funct3})
                ...
                {7'b0101000, 3'b000}: FPUSel = 5'b00000; // FDIV.S (não implementado)
                ...
                default: FPUSel = 5'b00000; // Operação inválida
            endcase
        end
        // --- Fim: Lógica ORIGINAL ---

        
        // --- Início: CORREÇÃO DO BUG 1 (Lógica de escrita) ---
        // Lógica de controle de escrita (depende de funct3/funct7)
        
        // 1. Pega os valores padrão do maindec
        RegWrite = md_RegWrite;
        FPURegWrite = md_FPURegWrite;
        ResultSrc = md_ResultSrc;

        // 2. Sobrescreve a lógica para FPU R-type (opcode 7'b1010011)
        if (op == 7'b1010011) begin
            // fcvt.w.s (fp-para-int)
            if (funct7 == 7'b1100000 && funct3 == 3'b000) begin
                RegWrite = 1'b1;        // Escreve no RegFile X
                FPURegWrite = 1'b0;   // NÃO escreve no RegFile F
            ...
            // Outras operações FPU R-Type (fadd, fsub, fmul, fle, flt)
            else begin
                RegWrite = 1'b0;
                FPURegWrite = 1'b1;   // Escreve no RegFile F
                ResultSrc = 2'b00;    // (Não importa)
